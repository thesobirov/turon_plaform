...
add_circle.addEventListener("click", () => {
    ...
    desc.value = ""
    date.value = ""
    modal.action = `/add_news/${modal.dataset.type_id}`
    main_date2.innerHTML = ""
    days2.innerHTML = ""
    get_news(main_date2, days2)
    modal__big__block_h1.innerText = "Yangilik Qo'shish"
    send.innerText = "Qo'shish"
...
getNewData(next2, days2, main_date2, 1)
getNewData(back2, days2, main_date2, -1)
...
function renderEvent(events) {
        ...
        filtered_list.map((item, index) => {
            ...
            <div class="news_item news_item_active" data-info_id="${item.id}">
                    ...
                    <div class="box1">
                        ...
                        <img class="edit" src="../../static/icon/edit.png" alt="">
                    </div>
                    <div class="news_text">
                        <h1>${item.title}</h1>
                        <p>${item.desc}</p>
    ...
    edit_icon.forEach((elem, index) => {
        elem.addEventListener("click", () => {
            ...
            send.innerText = "O'zgartirish"
            delete_btn.style.display = "flex"
            delete_btn.href = `/delete/${filtered_list[index].id}`
        })
    })
...
delete_btn.addEventListener("click", () => {
    console.log(newData[currentId].id)
    const id = newData[currentId].id
    fetch('/delete/' + id, {
